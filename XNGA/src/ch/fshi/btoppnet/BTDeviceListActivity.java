...
public class BTDeviceListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferencesUtil.savePreferences(context, Constants.SP_RADIO_SCAN_DURATION,
				Constants.DEFAULT_SCAN_DURATION);
		SharedPreferencesUtil.savePreferences(context, Constants.SP_RADIO_SCAN_DURATION_ID,
				Constants.DEFAULT_SCAN_DURATION_ID);
		SharedPreferencesUtil.savePreferences(context, Constants.SP_RADIO_SCAN_INTERVAL,
				Constants.DEFAULT_SCAN_INTERVAL);
		SharedPreferencesUtil.savePreferences(context, Constants.SP_RADIO_SCAN_INTERVAL_ID,
				Constants.DEFAULT_SCAN_INTERVAL_ID);
		...
		// intent.putExtra(Constants.INTENT_EXTRA_DEVICE_MAC,
		// deviceList.get(position).getMAC());
			...
			case Constants.MESSAGE_WHAT_DATA:
				...
				try {
					...
					case Constants.PACKET_TYPE_TIMESTAMP_DATA:
						...
						Toast.makeText(context, "received data:" + json.getString(Constants.PACKET_DATA),
	...
	class BTServiceBroadcastReceiver extends BroadcastReceiver {
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothDevice.ACTION_FOUND.equals(action)) {
				...
				/*
				 * -30dBm = Awesome -60dBm = Good -80dBm = OK -90dBm = Bad
				 */
				...
				if (deviceIndex < 0) { // device not exist
					...
					BTConnectButtonOnClickListener onClickListener = new BTConnectButtonOnClickListener(btDevice,
							bluetoothHelper);
				...
				if (!devicesFound.contains(myBTDevice.getMac())) {
					...
					dbScanLog.setScanDuration(SharedPreferencesUtil.loadSavedPreferences(context,
							Constants.SP_RADIO_SCAN_DURATION, Constants.DEFAULT_SCAN_DURATION));
				...
				// Get the BluetoothDevice object from the Intent
				mScanning = true;
				Log.d(Constants.TAG_APPLICATION,
						"Discovery process has been started: " + String.valueOf(System.currentTimeMillis()));
				if (System.currentTimeMillis() - scanStartTimestamp > SharedPreferencesUtil.loadSavedPreferences(
						context, Constants.SP_RADIO_SCAN_DURATION, Constants.DEFAULT_SCAN_DURATION)) {
				...
				Log.d(Constants.TAG_APPLICATION,
						"Discovery process has been stopped: " + String.valueOf(System.currentTimeMillis()));
				...
				for (BTDevice device : deviceList) {
					if (goodDevices.contains(device.getMAC())) {
						...
						/**
						 * make decisions on whether to choose bluetooth to
						 * communicate OR to scan using WiFi-Direct
						 */
		...
		case REQUEST_BT_ENABLE:
			if (resultCode == RESULT_OK) {
				...
				discoverableIntent.putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION,
						RESULT_BT_DISCOVERABLE_DURATION);
