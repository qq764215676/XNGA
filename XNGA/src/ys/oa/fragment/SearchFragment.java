...
import ys.oa.fragment.SearchCaseFragment.AsyncLoader;
 ...
 * wf 搜索列表
 */
public class SearchFragment extends Fragment implements PullToRefreshAttacher.OnRefreshListener {
		...
		protected Integer doInBackground(String... params) {
			try {
				if ("load".equals(params[0]) || "refresh".equals(params[0])) {
					mPage = 1;
					datasetlist = PostHttp.PostXML(XmlPackage.packageSelect(mSearchTxt == null ? "from OA_NEWS"
							: "from OA_NEWS where NewsTitle = '" + mSearchTxt + "'", "" + EVERY_PAGE,
							"LastChangedTime DESC", "NewsTitle, Content, LastChangedTime", "SEARCHYOUNGCONTENT",
							new DocInfor("", "OA_NEWS"), true, false));
				} else if ("upload".equals(params[0])) {
					index = EVERY_PAGE * mPage;
					datasetlist = PostHttp.PostXML(XmlPackage.packageSelect(mSearchTxt == null ? "from OA_NEWS"
							: "from OA_NEWS where NewsTitle = '" + mSearchTxt + "'", "" + EVERY_PAGE,
							"LastChangedTime DESC", "NewsTitle, Content, LastChangedTime", "SEARCHYOUNGCONTENT",
							new DocInfor("", "OA_NEWS"), true, false, index.toString()));
					mPage++;
				}
