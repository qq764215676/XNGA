...
import ys.oa.activity.CollectGoodListActivity;
import ys.oa.adapter.ListCollectGoodAdapter;
 ...
 * wf 物品检查列表
 */
public class CollectGoodFragment extends Fragment implements PullToRefreshAttacher.OnRefreshListener {
	private ListCollectGoodAdapter mAdapter;
	private ListView mListView;
	private CollectGoodListActivity mActivity;
	private PullToRefreshAttacher mPullToRefreshAttacher;
	private LoadingFooter mLoadingFooter;

	public static CollectGoodFragment newInstance() {
		CollectGoodFragment fragment = new CollectGoodFragment();
		return fragment;
	}

	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View contentView = inflater.inflate(R.layout.fragment_demo_list, null);
		mActivity = (CollectGoodListActivity) getActivity();
		mListView = (ListView) contentView.findViewById(R.id.listView);
		View header = new View(mActivity);
		mPullToRefreshAttacher = ((CollectGoodListActivity) mActivity).getPullToRefreshAttacher();
	...
	private ArrayList<CollectGoodEnity> enityList, listAll;
	...
	class AsyncLoader extends AsyncTask<String, Integer, Integer> {
		...
		protected Integer doInBackground(String... params) {
			try {
				if ("load".equals(params[0]) || "refresh".equals(params[0])) {
					mPage = 1;
					datasetlist = PostHttp.PostXML(XmlPackage.packageSelect("from NLGA_ROAD_GOOD where username='"
							+ Constants.USERID + "'", "" + EVERY_PAGE, "updateTime DESC", "", "SEARCHYOUNGCONTENT",
							new DocInfor("", "NLGA_ROAD_GOOD"), true, false));
				} else if ("upload".equals(params[0])) {
					index = EVERY_PAGE * mPage;
					datasetlist = PostHttp.PostXML(XmlPackage.packageSelect("from NLGA_ROAD_GOOD where username='"
							+ Constants.USERID + "'", "" + EVERY_PAGE, "updateTime DESC", "", "SEARCHYOUNGCONTENT",
							new DocInfor("", "NLGA_ROAD_GOOD"), true, false, index.toString()));
					mPage++;
				}
				if (Constants.REQUESTSUCCESS.equals(datasetlist.SUCCESS)) {
					...
					enityList = new ArrayList<CollectGoodEnity>();
					CollectGoodEnity enity = null;
					int n = 0;
					for (int i = 0; i < nameList.size(); i++) {
						if ("name".equals(nameList.get(i))) {
							enity = new CollectGoodEnity();
							enity.setOwnerName(valueList.get(i));
						} else if ("CardId".equals(nameList.get(i))) {
							enity.setId(valueList.get(i));
						} else if ("GoodName".equals(nameList.get(i))) {
							enity.setGoodName(valueList.get(i));
		...
		protected void onPostExecute(Integer result) {
			...
			case 1:
				listAll = enityList;
				mAdapter = new ListCollectGoodAdapter(mActivity, listAll);
