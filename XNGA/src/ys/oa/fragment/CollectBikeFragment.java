...
import ys.oa.activity.CollectBikeListActivity;
import ys.oa.adapter.ListCollectBikeAdapter;
 ...
 * wf 单车检查列表
 */
public class CollectBikeFragment extends Fragment implements PullToRefreshAttacher.OnRefreshListener {
	private ListCollectBikeAdapter mAdapter;
	private ListView mListView;
	private CollectBikeListActivity mActivity;
	private PullToRefreshAttacher mPullToRefreshAttacher;
	private LoadingFooter mLoadingFooter;

	public static CollectBikeFragment newInstance() {
		CollectBikeFragment fragment = new CollectBikeFragment();
		return fragment;
	}

	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View contentView = inflater.inflate(R.layout.fragment_demo_list, null);
		mActivity = (CollectBikeListActivity) getActivity();
		mListView = (ListView) contentView.findViewById(R.id.listView);
		View header = new View(mActivity);
		mPullToRefreshAttacher = ((CollectBikeListActivity) mActivity).getPullToRefreshAttacher();
	...
	class AsyncLoader extends AsyncTask<String, Integer, Integer> {
		...
		protected Integer doInBackground(String... params) {
			try {
				if ("load".equals(params[0]) || "refresh".equals(params[0])) {
					mPage = 1;
					datasetlist = PostHttp.PostXML(XmlPackage.packageSelect("from NLGA_ROAD_BICYCLE where username='"
							+ Constants.USERID + "' and VehicleType='" + Constants.vehicleType + "'", "" + EVERY_PAGE,
							"updateTime DESC", "", "SEARCHYOUNGCONTENT", new DocInfor("", "NLGA_ROAD_BICYCLE"), true,
							false));
		...
		protected void onPostExecute(Integer result) {
			...
			case 1:
				listAll = enityList;
				mAdapter = new ListCollectBikeAdapter(mActivity, listAll);
