...
import ys.oa.fragment.CollectCarFragment;
...
public class CollectCarListActivity extends FragmentActivity {
	private CollectCarFragment mContentFragment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if ("汽车检查".equals(title)) {
			Constants.vehicleType = "2";
		} else if ("摩托车检查".equals(title)) {
		...
		setTitle(title);
		mContentFragment = CollectCarFragment.newInstance();
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode != Activity.RESULT_OK)
			return;
		ToastMsg.showMsgTop(CollectCarListActivity.this, "上传成功");
		switch (requestCode) {
		case 0:
